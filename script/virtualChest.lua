...
local function removeVirtualChestFromInventory(inv)
	...
	for i = 1, #inv do
		local stack = inv[i]
		if stack.valid_for_read and stack.name == VIRTUAL_CHEST_PROTOTYPE then
			stack.clear()
		end
	end
end
...
local function removeVirtualChestFromEntityInventories(entity)
	...
	if entity.name == VIRTUAL_CHEST_PROTOTYPE then return end
...
local function purgeVirtualChestItems()
	for _, player in pairs(game.players) do
		if not player.valid then goto continue_player end
		for _, inv_def in pairs(defines.inventory) do
			removeVirtualChestFromInventory(player.get_inventory(inv_def))
		end
		local cursor = player.cursor_stack
		if cursor and cursor.valid_for_read and cursor.name == VIRTUAL_CHEST_PROTOTYPE then
			cursor.clear()
		end
		::continue_player::
	end
	...
	for _, surface in pairs(game.surfaces) do
		for _, entity in ipairs(surface.find_entities_filtered{ type = "item-entity" }) do
			if entity.valid and entity.stack.valid_for_read
					and entity.stack.name == VIRTUAL_CHEST_PROTOTYPE then
				entity.destroy{ raise_destroy = false }
			end
		end

		for _, ghost in ipairs(surface.find_entities_filtered{ type = "entity-ghost" }) do
			if ghost.valid and ghost.ghost_name == VIRTUAL_CHEST_PROTOTYPE then
...
function VirtualChest.purgeAll()
	for _, surface in pairs(game.surfaces) do
